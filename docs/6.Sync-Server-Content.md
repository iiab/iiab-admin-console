...
`moddir` must match the module folder name. The source server fills in missing
...
The source server calculates `ksize` in KiB from the module's files when it
generates the inventory. Any `ksize` value in `.iiab-meta` is replaced by the
calculated value.
