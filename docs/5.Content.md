...
Kolibri provides offline access to educational content, including Khan Academy videos, across many languages and subjects. Channels are downloaded from the Kolibri Content Library.

Kolibri content is installed through Quick Pick presets, which specify channels by ID. The Admin Console enables the Kolibri service automatically if it is not already active before downloading begins. Kolibri content is stored in `/library/kolibri/`.

### Quick Pick Presets

Quick Pick provides ready-made content bundles for common IIAB deployments. Selecting a preset installs all listed content (ZIMs, OER2Go modules, OSM maps, and Kolibri channels) and configures the home menu.

You can also save your own installed content as a preset using the Create a Content Collection panel under Install Content. This preset can then be reused to replicate the same content on other boxes.

## Document Version

This document was last maintained on May 20, 2026.
